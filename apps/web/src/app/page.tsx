...
const TICKER_ITEMS = [
  "@mirabuilds · Neckline · $420 by Nodeflux",
  "@samir.eth · Front door · $610",
  "team rektangle · Chest · $310",
  "@dev.drives · Rear door · won $340",
  "@kr1shna · Sleeve · $260",
  "Buildoors · Pocket · $120",
  "@mirabuilds · Skirt center · bought $540",
];

export default function LandingPage() {
  const [heroCd, setHeroCd] = useState("2d 05:12:08");

  useEffect(() => {
    const end = Date.now() + (2 * 86400 + 5 * 3600 + 12 * 60) * 1000;
    const interval = setInterval(() => {
      setHeroCd(formatCountdown(end).text);
    }, 1000);
    return () => clearInterval(interval);
  }, []);

  const miraListing = FIXTURE_LISTINGS[0];
  const samirListing = FIXTURE_LISTINGS[1];
  const rektListing = FIXTURE_LISTINGS[2];
        ...
        <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-8 items-center">
          ...
          <div className="relative max-w-[380px] mx-auto w-full py-4">
            ...
            <div className="w-full">
              <SurfaceFigure
                surface="outfit"
                patches={miraListing.patches.slice(0, 5)}
                mode="static"
                showPrices={false}
                animateDrop
              />
            ...
            <div className="absolute top-[12%] -left-[6%] bg-[var(--card)] border-2 border-[var(--line)] rounded-xl px-3 py-1.5 text-xs shadow-[3px_3px_0_var(--shadow)] flex items-center gap-2 animate-bounce select-none">
              <span className="w-6 h-6 rounded-lg bg-[var(--accent-soft)] text-[var(--accent-text)] grid place-items-center">
                <Bolt className="w-3.5 h-3.5" />
              </span>
              <span>
                Nodeflux bid <b className="font-mono">$420</b>
              </span>
            </div>

            <div className="absolute top-[48%] -right-[8%] bg-[var(--card)] border-2 border-[var(--line)] rounded-xl px-3 py-1.5 text-xs shadow-[3px_3px_0_var(--shadow)] flex items-center gap-2 select-none">
              <span className="w-6 h-6 rounded-lg bg-[var(--green-soft)] text-[var(--green)] grid place-items-center">
                <Check className="w-3.5 h-3.5" />
              </span>
              <span>
                Skirt center <b>bought</b>
              </span>
            </div>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 py-16 space-y-8">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <Card className="p-4 flex flex-col gap-4">
            <div className="bg-[var(--stage)] rounded-xl p-4 grid place-items-center min-h-[220px]">
              <div className="w-[140px]">
                <SurfaceFigure surface="outfit" patches={miraListing.patches} mode="static" showPrices={false} />
              </div>
            </div>
            <div>
            ...
            <Link href="/mirabuilds/mira" className="mt-auto">
          ...
          <Card className="p-4 flex flex-col gap-4">
            <div className="bg-[var(--stage)] rounded-xl p-4 grid place-items-center min-h-[220px]">
              <div className="w-full">
                <SurfaceFigure surface="car" patches={samirListing.patches} mode="static" showPrices={false} />
              </div>
            </div>
            <div>
              <h3 className="text-2xl font-bold flex items-center gap-2">
            ...
            <Link href="/samir.eth/samir" className="mt-auto">
              <Button size="small" className="gap-1.5">
                See a live car
                <ArrowRight className="w-3.5 h-3.5" />
              </Button>
          ...
          <Card className="p-4 flex flex-col gap-4">
            <div className="bg-[var(--stage)] rounded-xl p-4 grid place-items-center min-h-[220px]">
              <div className="w-[180px]">
                <SurfaceFigure surface="hoodie" patches={rektListing.patches} mode="static" showPrices={false} />
              </div>
            </div>
            <div>
              <h3 className="text-2xl font-bold flex items-center gap-2">
            ...
            <Link href="/rektangle/rekt" className="mt-auto">
              <Button size="small" className="gap-1.5">
                See a live hoodie
                <ArrowRight className="w-3.5 h-3.5" />
              </Button>
