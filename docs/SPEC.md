...
- Click a patch → selects it. "Bid" opens the **bid sheet**: brand name, logo upload (previewed live on the patch), amount with +10/+50/+100 and Buy now, perks (gasless, one signature, auto-refund), Place bid, "Pay with card", "Pay from another chain" (Aurora).
...
3. **Bidding** (brand): gasless, approve+bid in one click via Privy batching (or USDC permit). Pay with card (Privy onramp) or from any chain (Aurora).
